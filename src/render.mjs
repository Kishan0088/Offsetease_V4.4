...
import { home, supply, eac, esg, about, contact, sources } from './data/pages.mjs';
...
import {
  ...
  faq, closeCta, proof, statStrip, btn, rail, sunburst, ARROW,
      ...
      section(
        raw(
          str(head({ eyebrow: d.standards.eyebrow, headline: d.standards.headline })) +
            '<div class="cols cols--2" style="gap:clamp(26px,3.4vw,48px)">' +
            str(pills('Registries & standards', d.standards.registries)) +
            str(pills('Integrity frameworks', d.standards.frameworks)) +
            '</div>'
        ),
...
export function renderEsg() {
  ...
  const body = [
    ...
    str(section(raw(index), { tone: 'on-bone', id: 'services', chapter: 'Services' })),
...
export function renderContact() {
  ...
  const body = [
    str(hero(d.hero, { crumbs: [{ name: 'Contact', href: d.path }] })),
    str(
      section(
        raw(
          '<div class="cols cols--2" style="align-items:start;gap:clamp(32px,5vw,80px)">' +
            `<div class="reveal">${form}</div>` +
            '<div class="reveal">' +
            '<p class="label">How to reach us</p>' +
            details +
            `<div style="margin-top:clamp(26px,3.4vw,42px)">${str(
...
export function renderSources() {
  ...
  const list = d.items
    .map(
      (s) =>
        `<li><span class="sources__n">${String(s.n).padStart(2, '0')}</span><div>` +
        `<p class="sources__t">${esc(s.name)}</p>` +
  ...
  const body = [
    ...
    str(
      closeCta(
        {
          headline: s.cta,
          body: `Tell us where you are and we will map the shortest defensible route through it. ${site.responsePromise}`,
          photo: s.photo,
        },
        { secondary: { label: 'All ESG services', href: esg.path } }
      )
